...
| `git push` times out connecting to `github.com:443` (e.g. `20.205.243.166:443`) while `curl -x <proxy>` to GitHub works | A scoped empty proxy override forces a direct connection. See **PUSH BLOCKED BEHIND PROXY (Windows)** below. |
| `git push` hangs ~55s after a `401 Unauthorized`, or errors `cannot spawn git-askpass.exe` / `git credential-helper-selector get` | A bundled `helper-selector` credential helper hangs with no TTY. See **PUSH BLOCKED BEHIND PROXY (Windows)** below. |

---

## PUSH BLOCKED / HANGS BEHIND A PROXY (Windows git)

Symptom: `git push`/`fetch` to `github.com` hangs or times out, but `curl -x http://<proxy> https://github.com` returns `200` and an authenticated `curl -H "Authorization: Bearer <pat>" https://api.github.com/user` returns `200`. Two independent misconfigurations cause this on Windows hosts whose only path to GitHub is an HTTP proxy:

### Bug 1 — Empty scoped proxy override forces a direct connection

A `git config` entry of the form `http.https://github.com.proxy = ""` (empty string) **overrides** the working global `http.proxy` for that host, and an empty value means "no proxy" → git connects directly to `github.com:443`, which times out. This is invisible in `git config --global http.proxy` (which still shows the correct value) but shows up in the full regex dump.

**Diagnose:**
```bash
git config --get-regexp 'http\..*proxy'          # look for an empty-scoped entry
git ls-remote https://github.com/<owner>/<repo>.git   # works (GET via proxy)
GIT_CURL_VERBOSE=1 git push <remote> main 2>&1 | grep -iE 'connect to|Failed to connect'
# -> "connect to 20.205.243.166 port 443 ... Timed out"  == direct, NOT proxy
```

**Fix:** unset the stray scoped override (check both global and local scopes):
```bash
git config --global --unset-all 'http.https://github.com.proxy'
git config --local  --unset-all 'http.https://github.com.proxy'
# ensure the real proxy is honored:
...
Some bundled Windows git installs (e.g. the one shipped inside Hermes at `…/hermes/git/etc/gitconfig`) set `credential.helper=helper-selector`. On a credential prompt, git runs `git credential-helper-selector get`, which expects an interactive TTY that does not exist in an agent/background session → it hangs ~50s, then (if GCM is also configured) `git credential-manager get` finally runs and succeeds. Result: push appears to hang or fails with `cannot spawn git-askpass.exe` / `/dev/tty: No such device`.

**Diagnose:**
```bash
git config --show-origin --get-all credential.helper
# shows a line from .../hermes/git/etc/gitconfig  helper-selector   <-- the culprit
```

**Fix:** bypass the selector by setting the credential helper **locally for the repo** to GCM directly (GCM reads the PAT already stored in Windows Credential Manager — verify with `git-credential-manager get` after feeding `protocol=https\nhost=github.com`). Setting it locally prevents the system-scoped selector from running first:
```bash
git config --local credential.helper ""      # clear any local list
git config --local --add credential.helper manager
```

### Full proven push sequence (proxied Windows host, GCM + stored PAT)
```bash
cd /path/to/repo
git config --global http.proxy  http://127.0.0.1:40000
git config --global https.proxy http://127.0.0.1:40000
git config --global --unset-all 'http.https://github.com.proxy'   # Bug 1 fix
git config --local credential.helper ""                           # Bug 2 fix start
git config --local --add credential.helper manager
GIT_TERMINAL_PROMPT=0 git push <remote> main       # non-interactive; GCM supplies PAT
# Verify: git status -sb shows no "ahead"; remote HEAD == local HEAD
```
Note: `git ls-remote`/`curl` succeed through the proxy even before Bug 2 is fixed, because they don't trigger the credential selector the same way a write (push) does — so a passing `ls-remote` does NOT prove push will work. Always test with an actual `push` (or a `GIT_TERMINAL_PROMPT=0` dry run) end-to-end. See `references/windows-proxy-push-blocked.md` for the full transcript.
