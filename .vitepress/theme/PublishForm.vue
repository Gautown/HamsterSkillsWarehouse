...
<template>
  <div class="publish-form">
    <h2>发布技能</h2>
    <p class="pub-hint">
      发布后写入站内库 <code>.custom-skills/</code> 并自动重建站点 —— 新技能立即出现在分类、标签与搜索中。
    </p>
    ...
    <label class="pub-field">
      <span>正文 *（SKILL.md 的 Markdown 正文，发布后即为详情页内容）</span>
      <textarea v-model="body" rows="12" placeholder="# 用途说明&#10;&#10;## 使用方法&#10;……（Markdown，支持代码块）" />
    </label>

    <div class="pub-actions">
      <button type="button" class="pub-submit" :disabled="!canSubmit" @click="submit">
        {{ submitting ? '发布中…（自动重建约 30 秒）' : '发布技能' }}
      </button>
    </div>

    <div v-if="result" class="pub-result" :class="result.ok ? 'ok' : 'err'">
      <template v-if="result.ok">
        <p>✓ {{ result.message }}</p>
        <p>
          新页面：<a :href="result.pageUrl">{{ result.pageUrl }}</a>
          <span class="dim">（重建完成后可访问）</span>
        </p>
      </template>
